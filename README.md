...
The command report_net -connection _11343_ gives details about the net.

![sta_rpt_net](https://user-images.githubusercontent.com/53702961/106371605-7d148480-638c-11eb-9911-2f1236589cf1.png)

Now, we must replace the `buf_1` in the Driver pin with `buf_4` (Large size Buffer) by the command : `replace_cell _41881_ sky130_fd_Sc_hd__buf4`.

To check reports the command used was: `report_checks -fields {net cap slew input_pins} -digits 4`

By doing this iteratively, we reduced the slack to:-
![final_slack](https://user-images.githubusercontent.com/53702961/106371730-d8934200-638d-11eb-8542-7026215b7cd5.png)

### Verilog File Update

The command `write_verilog <.v file_location>` is used to update the verilog file after reduction in slack violations.

### CTS

After re-running Synthesis, Floorplan & Placement, to run Clock Tress Synthesis in OpenLANE the command used was: `% run_cts`
...
In OpenROAD, the timing analysis is done by creating a .db database file. This database file is created from the post-CTS LEF and DEF files.

Upon invoking OpenROAD with the command `openroad`, the following commands are executed for further timing analysis:-
...
PDN is generated by using the command: `gen_pdn`.

The following information is generated by this command:-
![pdn_info](https://user-images.githubusercontent.com/53702961/106371984-21002f00-6391-11eb-822f-b1670055ccf2.png)

> Note: It's observed that the DEF File being read is the `picorv32a.cts.def file`, which was generated after the CTS stage.

The distribution of power in the chip is shown below (image taken from @nickson_jose's tutorials)
![pdn_diag](https://user-images.githubusercontent.com/53702961/106372034-9ff56780-6391-11eb-88ae-58e78199e8ea.png)

Here, the green portion represents the design, i.e, picorv32a, the red and blue streaks represent the VCC and GND aspects respectively. As seen, the power and ground parts of the circuit are tapped from their respective ports and vertically distribute themselves to all the necessary points of the design. The Horizontal straps are set aside for the standard cells that we integrate in the design. 

#### Checking the stage of the design

To check the current stage of the design, the command that was used was: `echo $::env(CURRENT_DEF)`. The result will be the latest `.def` file being used, i.e., `pdn.def`.

![pdn_stage](https://user-images.githubusercontent.com/53702961/106372186-3fffc080-6393-11eb-9d28-fff6922d9c8f.png)


### Global and Detailed Routing

OpenLANE uses TritonRoute as the routing engine for Detailed routing and FastRoute engine for Global routing. Routing consists of two stages:-

- Global Routing: Output is a set of guide for each of the nets.
- Detailed Routing: Metal traces are iteratively laid across the routing guides to physically implement the routing guides (as obtained from Global routing).

To check what routing strategy currently in use we used the command: `echo $::env(ROUTING_STRATEGY)`

> Note: TritonRoute offers us various routing strategies. ROUTING_STRATEGY from 0 to 3 uses Triton-13 engine which has faster runtime & consumes less memory but less QoR (more DRC errors) and ROUTING_STRATEGY 14 uses Triton-14 engine which has less DRC errors but has much greater runtime a& consumes more memory.
> We set the ROUTING_STRATEGY to 0 (default).

#### Run Routing

To run routing in OpenLANE the command used was: `run_routing`.

![routing](https://user-images.githubusercontent.com/53702961/106372347-c799ff00-6394-11eb-8a55-d4207b0b3f05.png)
...
> Note: SPEF EXTRACTOR is yet to be integrated in the present versions of OpenLANE. It is made available inside the `Desktop/work/tools/` directory.
...
The SPEF file is written in the `trial_run1/results/routing/` directory.
![spef_write](https://user-images.githubusercontent.com/53702961/106372563-e26d7300-6396-11eb-95d3-48c5b0a8b187.png)

## Contact Information

Sandipan Sinha- sandipansinha22@gmail.com
